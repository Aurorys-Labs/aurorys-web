...
function LargeCard({
	...
	return (
		...
		>
			...
			<div className="flex-1 flex flex-col justify-end z-10">
				...
				>
					<div
						...
						style={{
							background:
								"linear-gradient(135deg, rgba(167,139,250,0.15), rgba(0,232,160,0.1))",
							color: "var(--aurora-violet-solid)",
						}}
					>
						<IconComponent className="w-6 h-6" />
					</div>
					<h3 className="text-xl font-bold mb-2 text-[var(--text-stellar)]">
						{title}
					</h3>
			...
			<motion.div
				variants={{
					rest: { opacity: 0, height: 0, y: 10, filter: "blur(4px)" },
					hover: { opacity: 1, height: "auto", y: 0, filter: "blur(0px)" },
				}}
				transition={{ duration: 0.4, ease: [0.25, 0.1, 0.25, 1] }}
		...
		>
			<div className="flex-1 flex flex-col justify-end z-10">
				...
				>
					...
					<h3 className="text-base font-bold mb-1 text-[var(--text-stellar)]">
						{title}
					</h3>
			...
			<motion.div
				variants={{
					rest: { opacity: 0, height: 0, y: 8, filter: "blur(4px)" },
					hover: { opacity: 1, height: "auto", y: 0, filter: "blur(0px)" },
				}}
				transition={{ duration: 0.4, ease: [0.25, 0.1, 0.25, 1] }}
		...
		>
			...
			>
				<div
					...
					style={{
						background:
							"linear-gradient(135deg, rgba(0,232,160,0.08), rgba(167,139,250,0.08))",
						color: "var(--text-muted)",
					}}
				>
					<IconComponent className="w-4 h-4" />
				</div>
				<h4 className="text-sm font-semibold mb-1 text-[var(--text-stellar)]">
					{title}
				</h4>
			</motion.div>
			<motion.div
				variants={{
					rest: { opacity: 0, height: 0, filter: "blur(2px)" },
					hover: { opacity: 1, height: "auto", filter: "blur(0px)" },
				}}
				transition={{ duration: 0.3, ease: [0.25, 0.1, 0.25, 1] }}
				...
				<div className="text-center mb-16">
					<h2 className="font-sans font-bold text-4xl md:text-5xl mb-4 tracking-tight">
						...
						<span className="text-gradient-aurora">
							{sectionTitle.split("The ")[1] || sectionTitle}
						</span>
					</h2>

...
const radialNodes = [
	...
	{ name: "Security Paths", href: "/security-paths", icon: Route },
	{ name: "Trust Center", href: "/trust", icon: Sparkles },
];
...
export function MobileNav() {
	...
	const handlePointerMove = (e: React.PointerEvent) => {
		...
		for (let i = 0; i < radialNodes.length; i++) {
			...
			if (diff < minDiff && diff < Math.PI * 0.15) {
				// Threshold to snap
				minDiff = diff;
				closestIdx = i;
			}
				...
				{isOpen && (
					...
					>
						...
						<div className="flex-1 overflow-y-auto pr-2 pb-16">
							<div className="grid grid-cols-2 gap-3">
								{radialNodes.map((node, idx) => (
									...
									>
										...
										<span className="font-medium text-[15px] leading-tight tracking-tight">
											{node.name}
										</span>
									</motion.a>
								))}
							...
							>
								<RainbowButton
									className="w-full justify-center py-6 rounded-2xl text-base"
									asChild
								>
						...
						>
							{isOpen ? (
								...
								<img
									src="/images/logos/normal updated colors.svg"
									alt="Menu"
									className={`w-8 h-8 transition-opacity ${isRadialOpen ? "opacity-50" : "opacity-100"}`}
								/>
							)}
						</motion.div>
					</div>
